...
public class BookJpaController implements Serializable {
  ...
  public List<Book> getTop5Books(){
      ...
      int x = (int) (Math.random() * 50 + 1);
        ...
        TypedQuery<Book> query = em.createNamedQuery("Book.findAll", Book.class).setMaxResults(8).setFirstResult(x);
  ...
  public List<Book> getBookByGenre(String genre) {
    ...
    String quer = "select distinct bk from Book bk where bk.genre LIKE :crit";

    TypedQuery<Book> searchedBooks = em.createQuery(quer, Book.class);
    searchedBooks.setParameter("crit", "%" + genre + "%");

    logger.log(Level.INFO, "getBookByGenre>> " + searchedBooks.getResultList().size());
...
//        query.setParameter("genre", genre);
  ...
  public List<Book> searchBooks(String criteria) {
    ...
    String quer = "select distinct bk from Book bk inner join bk.authorList bkl inner join bk.publisherId bkp where bk.isbn LIKE :crit OR bk.title LIKE :crit OR bkl.firstName LIKE :crit OR bk.genre LIKE :crit "
